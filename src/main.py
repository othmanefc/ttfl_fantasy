...
def get_end_date(season_end, date):
    maxx = np.maximum(season_end, date)
    if maxx == season_end:
        return season_end
    else:
        return datetime.datetime.strptime(date - datetime.timedelta(days=1),
                                          '%Y%m%d')
...
def append_df(initial_df, players_df, write=True):
    ...
    assert (players_df.date.max() -
            initial_df.date.max() == datetime.timedelta(days=1))
...
def get_df_prediction(date, season, season_year):
    if check_season == 'Season available':
        ...
        end = get_end_date(season_end_dt, date)
