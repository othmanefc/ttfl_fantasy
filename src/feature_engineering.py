...
from constants import DATA_DIR, METRICS, TEAM_ID, MAIN_DIR
...
def team_data_to_date_cache(data, team, to_date, metrics, add_cols=[]):
    ...
    if df.empty:
        ...
        df_empty.loc[0, ['date', 'date_dt', 'team', 'team_id'
                         ]] = to_date, to_date_dt, team, data.loc[0, 'team_id']
        return df_empty
...
class Season(object):
    ...
    def base_stats(self, data):
        ...
        data['fgm'] = data['fga'] - data['fg'] if np.all(
            ~data['fg'].isna()) else np.nan
        data['fg3m'] = data['fg3a'] - data['fg3'] if np.all(
            ~data['fg3'].isna()) else np.nan
        data['ftm'] = data['fta'] - data['ft'] if np.all(
            ~data['ft'].isna()) else np.nan
